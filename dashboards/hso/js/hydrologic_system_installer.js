...
function addNewComboOverlay(parentID, requirements) {
    ...
    appendChains(
        installer.getConnector().post("event/editoroverlay/set_prequel", null, [], (_d, _u, _qp, params) => {
        ...
        installer.getConnector().post("event/editoroverlay/set_combo_formula", null, [], (_d, _u, _qp, params) => {
            params.push(installer[idVar]);
            params.push(requirements.formula);
        }),
        installer.getConnector().post("event/editoroverlay/set_name", null, [], (_d, _u, _qp, params) => {
            params.push(installer[idVar]);
            params.push(requirements.name);
        }),
        installer.getConnector().post("event/editoroverlay/set_parent", null, [], (_d, _u, _qp, params) => {
            params.push(installer[idVar]);
...
function addNewOverlay(type, idVar) {
    ...
    appendChains(
        installer.getConnector().post("event/editoroverlay/add", null, [type]),
...
function addResultChildOverlay(overlay, resultType) {
    ...
    appendChains(
        ...
        installer.getConnector().post("event/editoroverlay/add_result_child", null, [], (_d, _u, _gp, params) => {
...
function addNewWaterLevelArea() {
    ...
    appendChains(
        installer.getConnector().get("items/settings/MAP_SIZE_M?", {
            token: app.token(),
            f: "JSON"
        }),
        ...
        installer.getConnector().post("event/editorarea/add", null, [1]),

        (data) => {
            installer[vars.DEFAULT_WATERAREA_ID] = Array.isArray(data) ? data[0] : data;
        },

        installer.getConnector().post("event/editorarea/add_polygons", { crs: "LOCAL" }, [], (_d, _u, _qp, params) => {
            params.push(installer[vars.DEFAULT_WATERAREA_ID]);
            params.push(installer[vars.DEFAULT_WATERAREA_MP]);
        }),

        installer.getConnector().post("event/editorarea/set_attribute", null, [], (_d, _u, _qp, params) => {
...
function setOverlayKey(overlay, key, value) {
    appendChains(
        ...
        installer.getConnector().post("event/editoroverlay/set_key_value", null, [], (_d, _u, _qp, params) => {
            params.push(overlay.id);
            params.push(key);
            params.push(value);
        })
...
function setResultType(resultType, idVar) {
    ...
    appendChains(
        ...
        installer.getConnector().post("event/editoroverlay/set_result_type", null, [], (_d, _u, _qp, params) => {
            params.push(installer[idVar]);
            params.push(resultType);
        })
    );
...
function setHsoOverlay(overlays, selectedOverlayID) {
    ...
    appendChains(
        ...
        installer.getConnector().post("event/editoroverlay/remove_attribute", null, [], (_d, _u, _qp, params) => {
...
function setRequiredOverlayAttribute(attributes, idVar) {
    ...
    appendChains(
        ...
        installer.getConnector().post("event/editoroverlay/set_attributes", null, [],
            (_d, _u, _qp, params) => {
...
function updateOverlays(actionAfterRefresh) {
    ...
    appendChains(
        ...
        installer.getConnector().get("items/overlays?", {
            token: app.token(),
            f: "JSON"
        }),
...
function runInstallation() {
    ...
    installer.catch(error => appendFeedback("Failed installing! Error: " + error));
...
function setupHsoComboOverlays() {
    ...
    let requiredCombos = [
...

        {
            ...
            resultChildTypeA: 'SURFACE_EVAPORATION',
            resultChildTypeB: 'BASE_TYPES',
            formula: 'SWITCH(BT, 0, 0, 0, 1, 0, 2, AT, 3, AT, 4, 0, 5, AT, 6, AT, 7, AT, 8, 0, 9, AT, 10, 0, 11, AT, 12, AT, 13, 0, 14, 0, 15, AT, 16, AT, 17, 0, 18, 0)'
        ...
        () => {
            ...
            for (let requiredCombo of requiredCombos) {
                ...
                if (combo != null) {
                    ...
                    appendChains(
                        installer.getConnector().post("event/editoroverlay/remove_attribute", null, params));
...
function setupHsoWaterLevelAreas() {
    ...
    appendChains(
        ...
        installer.getConnector().get("items/areas-" + key + "?", {
            token: app.token(),
            f: "JSON"
        }),
...
function requestAreaSetup() {
    ...
    appendChains(
        installer.getConnector().get("items/areas?", {
            token: app.token(),
            f: "JSON"
        }),
...
function updatePanels(actionAfterRefresh) {
    ...
    appendChains(
        ...
        installer.getConnector().get("items/panels?", {
            token: app.token(),
            f: "JSON"
        }),
...
function setHsoTemplatePanel() {
    ...
    appendChains(
        installer.getConnector().post("event/editorpanel/set_attribute", null, [], (_d, _u, _qp, params) => {
            params.push(installer[vars.DASHBOARD_PANEL_ID]);
            params.push(HSO_PANEL_ATTRIBUTE);
            params.push(1.0);
        }),

        installer.getConnector().post("event/editorpanel/remove_attribute", null, [], (_d, _u, _qp, params) => {
        ...
        installer.getConnector().post("event/editorpanel/set_template_panel_maplink", null, [], (_d, _u, _qp, params) => {
            params.push(installer[vars.DASHBOARD_PANEL_ID]);
            params.push("AREAS");
        }),

        installer.getConnector().post("event/editorpanel/set_template_attribute", null, [], (_d, _u, _qp, params) => {
            let hsoOverlay = installer[vars.HSO_OVERLAY];
            params.push(installer[vars.DASHBOARD_PANEL_ID]);
            let attribute = hsoOverlay.keys[WATER_LEVEL_KEY] == null ? WATER_LEVEL_KEY : hsoOverlay.keys[WATER_LEVEL_KEY];
            params.push(attribute);
...
function addAndSetNewTemplateTextPanel() {
    ...
    appendChains(
        installer.getConnector().post("event/editorpanel/add", null, ["TEMPLATE_TEXT_PANEL"]),

        (data) => {
            installer[vars.DASHBOARD_PANEL_ID] = Array.isArray(data) ? data[0] : data;
        },

        installer.getConnector().post("event/editorpanel/set_size", null, [], (_d, _u, _qp, params) => {
            params.push(installer[vars.DASHBOARD_PANEL_ID]);
            params.push(PANEL_WIDTH);
            params.push(PANEL_HEIGHT);
        }),

        installer.getConnector().post("event/editorpanel/set_name", null, [], (_d, _u, _qp, params) => {
            params.push(installer[vars.DASHBOARD_PANEL_ID]);
...
function setDashboardContent() {
    ...
    appendChains(
        ...
        installer.getConnector().post("event/editorpanel/set_text", null, [], (_d, _u, _qp, params) => {
            params.push(installer[vars.DASHBOARD_PANEL_ID]);
        ...
        installer.getConnector().post("event/editorpanel/apply_template_panels", null, [], (_d, _u, _qp, params) => {
            params.push(installer[vars.DASHBOARD_PANEL_ID]);
        }),
...
function requestUpdateIndicators() {
    ...
    yesButton.onclick = () => {
        ...
        appendChains(
            ...
            installer.getConnector().post("event/editor/update", null, [true]),
