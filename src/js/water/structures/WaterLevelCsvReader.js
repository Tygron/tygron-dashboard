class WaterLevelCSVReader {
	...
	_processHeader(lineNumber, line) {
		...
		for (let i = 0; i < headers.length; i++) {
			...
			if (lineNumber == 0) {
				this._includes.push(i == 0 || this._headerPredicate == null ? true : this._headerPredicate(headers[i]));
			}
			if (this._includes[i]) {
				headerLine.push(headers[i]);
			}
		}
		this._results.push(headerLine);
	}
	...
	_processDateLine(line) {
		...
		let resultLine = [];
		for (let j = 0; j < data.length && j < this._includes.length; j++) {
			if (this._includes[j]) {
				resultLine.push(data[j]);
			}
		}
		this._results.push(resultLine);
