...
/**
 * @param {*} timeframeSlider element created by @function addTimeframeSlider
 * @param {*} timeframe for slider value
 * @param {*} timeframes maximum number of timeframes
 * @param {*} onInput function to call 
 */
export function setupTimeframeSlider(timeframeSlider, timeframe, timeframes, onInput, timeframeNames) {
    ...
    if (sliderInput != null) {
        ...
        sliderInput.oninput = function() {
			...
			let textValue = Array.isArray(timeframeNames) && sliderInput.value < timeframeNames.length ? timeframeNames[sliderInput.value] : sliderInput.value;
			 
			timeframeSlider.style.setProperty('--text-value', JSON.stringify(textValue))
