...
export class QueryDataManager {
	...
	/**
	 * * @param {boolean} forceToArray wrap the data in an array if it isn't
		 * @param {boolean} forceArrayToArrays Wrap the array in an array if it isn't. Requires forceToArray to be true.
		 */
	getData(key, forceToArray = true, forceArrayToArrays = false) {
		let queryDataObject = this.getQueryDataObject(key);
		let data = queryDataObject.getData(this.allowFallbackData);
		data = forceToArray ? ArrayUtils.coerceToArray(data) : data;
		if(forceToArray && forceArrayToArrays){
